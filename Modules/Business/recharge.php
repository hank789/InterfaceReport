...
// 新方式: 从自己记录的数据源里读
function recharge($module, $interface, $date, $start_time, $offset, $count) {
    ...
    $rechargeData = get_business_recharge_data($date);
    $rechargeCallbackData = get_business_recharge_callback_data($date);
    foreach($rechargeData as $time_point => $row) {
        $all_count = $success_count = $failed_count = 0;
        $all_money = $success_money = $failed_money = 0;
        $time = $row['time'];
        $success_count = $row['suc_count'];
        $failed_count = $rechargeCallbackData[$time_point]['suc_count'];
        $success_money = $row['suc_money'];
        $failed_money = $rechargeCallbackData[$time_point]['suc_money'];
        $html_class = '';
        if ($success_count > 0) {
    ...
    include ST_ROOT . '/Views/Business/recharge.tpl.php';
    include ST_ROOT . '/Views/Includes/footer.tpl.php';
}

function format_business_recharge($st_explode, $date, $stepSeconds = 300)
{
...
// allData
function get_business_recharge_data($date, $stepSeconds = 300)
{
    $data = [];
    $file = Config::$dataPath . Common::STAT_BUSINESS_RECHARGE_DIR . $date;
    if(is_readable($file)) {
        $data = file($file);
    }

    return format_business_recharge($data, $date, $stepSeconds);
}

// allData
function get_business_recharge_callback_data($date, $stepSeconds = 300)
{
    $data = [];
    $file = Config::$dataPath . Common::STAT_BUSINESS_RECHARGE_CALLBACK_DIR . $date;
    if(is_readable($file)) {
        $data = file($file);
    }
