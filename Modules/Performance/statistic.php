...
function statistic($module, $interface, $date, $start_time, $offset)
{
	...
	for ($i = 13; $i >= 1; $i --) {
		...
		$date_btn_str .= '<a href="/?md=performance&fn=statistic&date=' . "$the_date&$query" . '" class="btn ' . $html_class . '" type="button">' . $html_the_date . '</a>';
		if ($i == 7) {
			$date_btn_str .= '</br>';
		}
	}
	...
	foreach (\Statistics\Lib\Cache::$modulesDataCache as $mod => $interfaces) {
		...
		$module_str .= '<li><a href="/?md=performance&fn=statistic&module=' . $mod . '">' . $mod . '</a></li>';
		if ($module == $mod) {
			$module_str .= '<ul>';
			foreach ($interfaces as $if) {
				$module_str .= '<li><a href="/?md=performance&fn=statistic&module=' . $mod . '&interface=' . $if . '">' . $if . '</a></li>';
			}
			$module_str .= '</ul>';
		}
	}
	...
	include ST_ROOT . '/Views/Performance/statistic.tpl.php';
