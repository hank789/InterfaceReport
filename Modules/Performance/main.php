...
function main($module, $interface, $date, $start_time, $offset)
{
	...
	unset($_GET['start_time'], $_GET['end_time'], $_GET['date'], $_GET['fn'],$_GET['md']);
	...
	if ($data) {
		...
		foreach ($data as $item) {
            ...
            $item['total_avg_time'] = $item['total_avg_time'];
			...
			$table_data = "\n<tr $html_class>
                       ...
                       <td>{$item['total_count']}</td>
                        ...
                        <td>" . ($item['fail_count'] > 0 ? ("<a href='/?md=performance&fn=logger&$query&start_time=" . (strtotime($item['time']) - 300) . "&end_time=" . (strtotime($item['time'])) . "'>{$item['fail_count']}</a>") : $item['fail_count']) . "</td>
	...
	for ($i = 13; $i >= 1; $i --) {
		...
		$date_btn_str .= '<a href="/?md=performance&date=' . "$the_date&$query" . '" class="btn ' . $html_class . '" type="button">' . $html_the_date . '</a>';
		if ($i == 7) {
			$date_btn_str .= '</br>';
		}
	}
	$the_date = date('Y-m-d');
	$html_the_date = $date == $the_date ? "<b>$the_date</b>" : $the_date;
	$date_btn_str .= '<a href="/?md=performance&date=' . "$the_date&$query" . '" class="btn" type="button">' . $html_the_date . '</a>';
	...
	if (empty(\Statistics\Lib\Cache::$ServerIpList)) {
		$notice_msg = <<<EOT
<h4>数据源为空</h4>
您可以 <a href="/?md=admin&fn=admin&act=detect_server" class="btn" type="button"><strong>探测数据源</strong></a>或者<a href="/?md=admin&fn=admin" class="btn" type="button"><strong>添加数据源</strong></a>
EOT;
	}
	...
	include ST_ROOT . '/Views/Performance/main.tpl.php';
...
function formatSt($str, $date, &$code_map, $stepSeconds = 300)
{
	...
	foreach ($st_data as $time_line => $item) {
		$data[$time_line] = array(
			...
			'total_avg_time' => $item['suc_count'] + $item['fail_count'] == 0 ? 0 : round(($item['suc_cost_time'] + $item['fail_cost_time']) / ($item['suc_count'] + $item['fail_count']), 9) * 1000,
			'suc_count' => $item['suc_count'],
			'suc_avg_time' => $item['suc_count'] == 0 ? $item['suc_count'] : round($item['suc_cost_time'] / $item['suc_count'], 9) * 1000,
			'fail_count' => $item['fail_count'],
			'fail_avg_time' => $item['fail_count'] == 0 ? 0 : round($item['fail_cost_time'] / $item['fail_count'], 9) * 1000,
			'precent' => $item['suc_count'] + $item['fail_count'] == 0 ? 0 : round(($item['suc_count'] * 100 / ($item['suc_count'] + $item['fail_count'])), 4),
			'suc_cost_time' => $item['suc_cost_time'],
			'fail_cost_time' => $item['fail_cost_time'],
		);
	}
...
// allData
function getData($module='AllData', $interface= 'Statistics') {
	$date = date('Y-m-d');
	multiRequestStAndModules($module, $interface, $date);
	$all_st_str = '';
	if (is_array(\Statistics\Lib\Cache::$statisticDataCache['statistic'])) {
